...
function ManagerMenu({ navigation }) {
    ...
    const renderAddNewItemCard = () => {
        return (
            <TouchableOpacity 
                style={styles.addNewItemCard} 
                onPress={() => navigation.navigate('ManagerEditMenu', { item: {} })}
            >
    ...
    const renderCard = ({ item: food }) => (
        <TouchableHighlight 
            underlayColor={'white'} 
            activeOpacity={0.9} 
            onPress={() => navigation.navigate('ManagerEditMenu', food)}>
...
,
   ...
   card:{
    ...
    // Styling for the add new item card
...
addNewItemText: {
    // Styling for the text inside the add new item card
    color: 'white',
    fontSize: 18,
    marginLeft: 10,
},
